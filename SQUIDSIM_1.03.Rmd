---
title: "SQUIDSIM, SQUID SIMULATOR, Version 1.3, SPRFMO SC13"
...
The version 1.0 was design to simulate the dynamic of Humboldt Squid population in Chilean waters (@Paya2019). The simulations were done through years using a monthly steps. The recruitments were modelled using a @Ricker1954 or a @BevHolt stock-recruitment relationship, with a steepness parameter (h) [@MaceDoonan1988] and process errors (rsigma). Recruitment could be multiply by a seasonal factor to generate a seasonal pattern. Seasonal pattern can be constant (no effect), exponential decreasing or sinusoidal. The fishing component includes two fleets. The fishing selectivity were modeled as double half-normal functions.
...
library(knitr)
if(gm==1){
  growth.pars<-data.frame(gm_a,gm_b,a,b)
} else if(gm==2) {
    growth.pars<-data.frame(loo,r,t0,a,b)
} else{data.frame(gm_a,gm_b,gm_c,a,b)}
...
### Figure 4b. M by age and YEAR.MONTH (if M is constant then one color).
```{r message=FALSE}
...
par(mar=c(4,4,1,1))
par(mfcol=c(4,1))
plot(tbins[aniomes],Totales$CPUE1[aniomes],typ="l",ylim=c(min(Totales$CPUE1[aniomes]),max(Totales$CPUE1[aniomes])),xlab="Year_Month",ylab="CPUE 1",col=2)
...
plot(tbins[aniomes],Totales$CPUE3[aniomes],typ="l",ylim=c(min(Totales$CPUE3[aniomes]),max(Totales$CPUE3[aniomes])),xlab="Year_Month",ylab="CPUE 3",col=2)
plot(tbins[aniomes],Totales$CPUE4[aniomes],typ="l",ylim=c(min(Totales$CPUE4[aniomes]),max(Totales$CPUE4[aniomes])),xlab="Year_Month",ylab="CPUE 4",col=2)
#
...
text(tbins[20],0.75*max(max(wm_m)),"- - - Fleet 4",col=5)
...
fish.prs<-data.frame(Fref1,as1,bs1,cs1,Fref2,as2,bs2,cs2,
                     Fref3,as3,bs3,cs3,Fref4,as4,bs4,cs4,q1,hyp1,q2,hyp2,q3,hyp3,q4,hyp4)
#popu.pars
...
```{r}
indices.prs<-data.frame(as5,bs5,cs5,as6,bs6,cs6,qacous1,qacous2,qMPH)
#popu.pars
